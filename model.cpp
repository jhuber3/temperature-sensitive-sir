...
struct sirmodel_starttemp
{
  ...
  sirmodel_starttemp(vector<double> parameters, double start_temp) : parameters(parameters), start_temp(start_temp) {}
...
struct sirmodel_uncertainty
{
  ...
  vector<double> mu_parms, vector<double> PDR_parms, vector<double> b_parms): parameters(parameters),
  EFD_parms(EFD_parms), pEA_parms(pEA_parms), MDR_parms(MDR_parms), a_parms(a_parms), pMI_parms(pMI_parms),
  mu_parms(mu_parms), PDR_parms(PDR_parms), b_parms(b_parms) {}

  void operator()( state_type &x , state_type &dxdt , double t ) const
  {
    double temp = parameters[0]*sin((2.0*pi/365.0)*(t))+parameters[1];
    double K = carrying_capacity(temp,29.0,0.05,20000, EFD_parms, pEA_parms, MDR_parms, mu_parms);
    ...
    dxdt[0] = (EFD(temp, EFD_parms)*pEA(temp, pEA_parms)*MDR(temp, MDR_parms))*(x[0]+x[1]+x[2])*(1-((x[0]+x[1]+x[2])/K))-
    (a(temp, a_parms)*pMI(temp, pMI_parms)*x[5]/(x[3]+x[4]+x[5]+x[6])+mu(temp, mu_parms))*x[0];

    dxdt[1] = (a(temp, a_parms)*pMI(temp, pMI_parms)*x[5]/(x[3]+x[4]+x[5]+x[6]))*x[0]-(PDR(temp, PDR_parms)+mu(temp, mu_parms))*x[1];

    dxdt[2] = PDR(temp, PDR_parms)*x[1]-mu(temp, mu_parms)*x[2];
...
int main(int argc, char*argv[]){
  ...
  //   integrate_const(stepper, sirmodel_starttemp(parms, parms[1]), x, 0.00, 365.00, 0.01,stream_writer( fout ));
  // }
  ...
  //     MDR_uncertainty[index], mu_uncertainty[index]);
  //     parms[0] = amplitude[jj];
  //     parms[1] = oscillation_temps[ii];
  //     state_type x = {0.985 * M_initial, 0.0, 0.015 * M_initial, 9999, 0, 1, 0};
  ...
  //       mu_uncertainty[index], PDR_uncertainty[index], b_uncertainty[index]),
  //     x, 0.0, 365.0, 0.01, stream_writer ( fout ));
  //     // integrate_adaptive(make_controlled( 1.0e-5 , 1.0e-5 , error_stepper_type() ),  sirmodel_uncertainty(parms, EFD_uncertainty[index], pEA_uncertainty[index],
  //     //   MDR_uncertainty[index], a_uncertainty[index], pMI_uncertainty[index],
  //     //   mu_uncertainty[index], PDR_uncertainty[index], b_uncertainty[index]),
  //     // x, 0.0, 365.0, 0.1, stream_writer ( fout ));
  //   }
  //   fout << '\n';
  // }
...
{
  return inverted_quadratic(temp, parms[2], parms[0], parms[1]);
}
  ...
  std::vector<double> EFD_parms, std::vector<double> pEA_parms, std::vector<double> MDR_parms, std::vector<double> mu_parms)
{
  ...
  double alpha = (EFD(T0, EFD_parms)*pEA(T0, pEA_parms)*MDR(T0, MDR_parms)-mu(T0, mu_parms))/(EFD(T0, EFD_parms)*pEA(T0, pEA_parms)*MDR(T0, MDR_parms));
